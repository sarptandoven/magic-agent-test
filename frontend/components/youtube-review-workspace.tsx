...
const PROVIDERS: Array<{
  ...
  { value: "yt_dlp", label: "yt-dlp", shortLabel: "yt-dlp", icon: Terminal },
];
    ...
    <main className="min-h-screen bg-candy text-[#26172f] selection:bg-pink-300/60 selection:text-[#26172f]">
      <header className="flex h-16 items-center justify-between border-b border-white/70 bg-white/65 px-5 backdrop-blur-xl">
        <div className="flex min-w-0 items-center gap-3">
          <div className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-rainbow text-white shadow-candy">
            <Youtube className="h-4 w-4" />
          </div>
          <div className="min-w-0">
            <h1 className="truncate text-sm font-bold">YouTube Prompt Review</h1>
            <p className="truncate text-xs text-[#7c6688]">Saved eval prompts, Data API, and yt-dlp</p>
          </div>
        </div>
        ...
        <section className="glass-panel grid min-h-0 grid-rows-[auto_1fr_auto] overflow-hidden rounded-[14px]">
          <div className="flex items-center justify-between border-b border-[#875caa24] bg-white/55 px-4 py-3">
            <div>
              ...
              <p className="mt-0.5 text-xs text-[#7c6688]">
                {batch ? `${batch.items.length} prompts loaded` : "No generated set loaded"}
              </p>
            </div>
            {summary.total > 0 && (
              <span className="rounded-full border border-[#875caa24] bg-white/70 px-2 py-1 text-[11px] font-bold text-[#563861]">
          ...
          <div className="custom-scrollbar min-h-0 space-y-3 overflow-y-auto p-4">
            {!batch ? (
              <div className="rounded-[14px] border border-[#875caa24] bg-white/75 p-4">
                <p className="text-sm font-bold text-[#392545]">Generate the saved prompt set.</p>
                <p className="mt-2 text-xs leading-5 text-[#7c6688]">
                  This queues a fresh Data API and yt-dlp run for every prompt in the eval JSONL. Videos appear here as each run completes.
                </p>
              </div>
            ) : (
                ...
                <button
                  ...
                  className={cn(
                    ...
                    selectedItem?.review_id === item.review_id
                      ? "border-[#875caa44] bg-white shadow-sm"
                      : "border-[#875caa24] bg-white/62 hover:bg-white/86",
                  )}
                >
                  <div className="flex items-center justify-between gap-2">
                    <p className="truncate text-sm font-black text-[#392545]">
          ...
          <div className="border-t border-[#875caa24] bg-white/45 p-3">
            <Button
              ...
              disabled={isBusy}
              className="h-10 w-full gap-2 rounded-xl bg-gradient-to-br from-rose-400 to-sky-400 text-white hover:opacity-90"
            >
          ...
          <div className="glass-panel rounded-[14px] p-4">
            {selectedItem ? (
              <div className="grid gap-3 lg:grid-cols-[1fr_auto]">
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    <h2 className="text-base font-black text-[#392545]">{readablePromptName(selectedItem.name)}</h2>
                  ...
                  <p className="mt-2 text-sm leading-6 text-[#392545]">{selectedItem.prompt}</p>
                </div>
              ...
              <div className="flex items-center justify-between gap-4">
                <div>
                  <h2 className="text-base font-black text-[#392545]">No prompt selected</h2>
                  <p className="mt-1 text-sm text-[#7c6688]">Generate a fresh set to start reviewing videos.</p>
                </div>
              </div>
            )}
          </div>
...
function ProviderReviewPanel({
  ...
  return (
    <article className="glass-panel grid min-h-[620px] grid-rows-[auto_minmax(260px,1fr)_auto] overflow-hidden rounded-[14px]">
      <div className="border-b border-[#875caa24] bg-white/55 px-4 py-3">
        <div className="flex items-center justify-between gap-3">
          <div className="flex min-w-0 items-center gap-2">
            <div className="grid h-9 w-9 shrink-0 place-items-center rounded-xl border border-[#875caa24] bg-white/80 text-[#563861]">
              <Icon className="h-4 w-4" />
            </div>
            <div className="min-w-0">
              <h2 className="truncate text-sm font-bold">{providerMeta.label}</h2>
              <p className="truncate text-xs text-[#7c6688]">{provider?.project_id ?? "Not started"}</p>
            </div>
          </div>
      ...
      <div className="space-y-3 border-t border-[#875caa24] bg-white/55 p-4">
        {error && (
        ...
        <label className="grid gap-2">
          <span className="flex items-center gap-1.5 text-xs font-bold text-[#7c6688]">
          ...
          <textarea
            ...
            className="min-h-[118px] rounded-[12px] border border-[#875caa24] bg-white/90 p-3 text-sm leading-6 text-[#26172f] outline-none focus:border-pink-300"
        ...
        <Button
          ...
          className="h-10 w-full gap-2 rounded-xl bg-[#26172f] text-white hover:bg-[#392545]"
        >
...
function Metric({ icon, label, value }: { icon?: ReactNode; label: string; value: string }) {
  return (
    <div className="min-w-0 rounded-[12px] border border-[#875caa24] bg-white/75 px-2 py-2">
      <p className="flex items-center gap-1 truncate text-[11px] font-bold text-[#8e7899]">
        {icon}
        {label}
      </p>
      <p className="mt-1 truncate text-xs font-black text-[#392545]">{value}</p>
    </div>
  );
}
