...
GENERIC_YOUTUBE_BROLL_PATTERN = re.compile(
    r"\b(vertical|shorts?|stock(?:\s+(?:video|footage))?|b[-\s]?roll|generic|police lights|street scene)\b",
    re.IGNORECASE,
)
...
OPENAI_PRODUCT_TERM_PATTERN = re.compile(
    r"\b(Codex|ChatGPT(?:\s+Atlas)?|GPT-\d(?:\.\d+)?|GPT-4o|O3|O4(?:[-\s]?mini)?|O1(?:[-\s]?(?:pro|mini))?|Sora|AgentKit|DALL-E)\b",
    re.IGNORECASE,
)
...
YOUTUBE_REVIEW_PROVIDERS: tuple[YouTubeReviewProvider, ...] = ("youtube_data_api", "yt_dlp")
...
class CreateProjectRequest(BaseModel):
    ...
    youtube_search_provider: YouTubeSearchProvider = "auto"
...
- For generic b-roll or stock-style visuals, append "stock", "stock video", or "stock footage".
...
Currentness rules:
- If the user asks for latest, today, current, breaking, or news, do not rely on stale model memory.
- Use ToolSearch before naming a specific current product, event, date, or claim.
...
def build_youtube_script_prompt(request: CreateProjectRequest, ctx: ProjectContext) -> str:
    ...
    return "\n".join(
        [
            ...
            "Return a concise title and ordered sections only.",
        ]
    )
...
def normalize_youtube_script_plan(plan: YouTubeScriptPlan) -> YouTubeScriptPlan:
    ...
    return YouTubeScriptPlan(title=title[:120], sections=sections)
...
def compact_openai_product_hint(source_prompt: str, hint: str) -> str:
    ...
    if (
        ...
        compacted = f"{compacted} official"
...
def factual_youtube_search_hint(source_prompt: str, hint: str, product_terms: list[str] | None = None) -> str:
    ...
    if "openai" in source_prompt.lower() and has_openai_product_term(cleaned_hint):
        return compact_openai_product_hint(source_prompt, cleaned_hint)
    if "openai" in source_prompt.lower() and product_terms:
            ...
            section.model_copy(
                update={
                    "search_hint": carry_source_subject_terms(
                        ...
                        if looks_like_youtube_creation_instruction(section.search_hint)
                        else compact_search_query(section.search_hint),
                    )
                }
            )
            for section in sections
...
def build_youtube_workflow_brief(request: CreateProjectRequest, ctx: ProjectContext) -> str:
    ...
    lines = [
        ...
        f"Resolution: {request.resolution}",
        (
            "YouTube search provider: auto. The backend uses yt-dlp relevance search by default and switches to YouTube Data API only for clearly recent/current/news/date-sensitive section queries."
            if request.youtube_search_provider == "auto"
            else f"YouTube search provider: {request.youtube_search_provider}. The backend will use this provider for clip search."
        ),
        (
            "YouTube provider fallback: enabled. If the requested provider returns no candidates or errors, the backend may try the other provider."
            if request.youtube_allow_provider_fallback
            else "YouTube provider fallback: disabled. The backend will only use the requested provider for clip search."
        ),
        duration_constraint_line(constraints),
        scene_constraint_line(constraints),
        f"Target final runtime: {budget.final_duration_seconds} seconds after crossfades.",
    ...
    if is_factual_youtube_prompt(request.prompt):
        lines.extend(
            [
                ...
                "Use ToolSearch first inside the prompt-based YouTube script planner before naming current products, events, dates, or claims.",
        ...
        lines.append("The prompt-based YouTube script planner will make literal search hints for real people/events/brands and stock-style hints only for generic b-roll.")
    lines.extend(
        [
    ...
    update_project_state(
        ...
        decision={
            ...
            "metadata": {
                ...
                "model": youtube_script_agent.model,
            },
        },
    )
    return plan
...
async def create_youtube_short_impl(
    ...
    *,
    ...
    if not youtube_allow_provider_fallback:
        download_kwargs["allow_provider_fallback"] = False
    clip_results = await download_youtube_clip_assets(ctx, sections, **download_kwargs)
    videos: list[dict[str, Any]] = []
    failures: list[dict[str, str]] = []
    ...
    try:
        final_video = await stitch_assets_per_section(ctx, scenes_for_stitch)
    except Exception as exc:
...
async def create_youtube_short_from_prompt_impl(
    ...
    *,
    ...
    return await create_youtube_short_impl(
        ...
        script.sections,
        token_output=pending_token_output(ctx, youtube_script_agent.model),
        proxy_url=proxy_url,
    )
...
youtube_script_agent = Agent(
    name="Notebook-Style YouTube Script Planner",
    model=ENV.get("YOUTUBE_SCRIPT_MODEL", ENV.get("OPENAI_FAST_MODEL", ENV.get("OPENAI_MODEL", "gpt-5.4"))),
    instructions=(
        "Draft only the structured YouTube script requested by the prompt. "
        "Use ToolSearch only for latest/current/breaking/news prompts before naming time-sensitive facts."
    ),
